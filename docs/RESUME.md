# RESUME — reef-sfm-mote-keys

Live status, product tables, ADRs in effect, AWS inventory, and open items.
Moved here from `CLAUDE.md` on 2026-09-25 (always-loaded context stays distilled;
see `~/code/CLAUDE.md` "Repo hygiene"). Keep this current as work lands.
Operational invariants and hard constraints stay in `CLAUDE.md`.
...
HARD CONSTRAINTS: see `CLAUDE.md` "Hard constraints" (cite, don't restate).
