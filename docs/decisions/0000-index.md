...
| 0028 | Corrected T1 AOI Z window [−9.2, +1.8] m, surface-median-anchored; supersedes 0026's window (decided Chat 5 — file pending on `fix/level-camera-nadir`, see CLAUDE.md) | Accepted (file pending) | 5 |
| 0029 | Full-area ortho PointCloudData-direct (`buildDem` hangs on 487M-pt cloud); portfolio visual, not the ESM Step-15 product (decided Chat 5 — file pending on `fix/level-camera-nadir`, see CLAUDE.md) | Accepted (file pending) | 5 |
| 0030 | Reconciliation metric scope: pure-Python tier (rugosity, standardized elevation, VRM) vs MultiscaleDTM stubs; 1 cm resample (file on open branch `feat/reconcile-metrics`) | Accepted (file on branch) | 6 |
