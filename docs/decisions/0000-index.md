...
| [0035](0035-step4-threshold-does-not-recover-registration.md) | Falsified: lowering step4 quality threshold 0.50→0.30 does NOT recover R2 registration. 131 cameras aligned at any threshold — registration ceiling is corpus geometry (out-and-back overlap), not threshold artifact. `frame_retention` separates (0.485→0.993); `registration_ratio` flat (0.482→0.485). 0.30 DSM proceeds as better-provenance run; `registration_ratio` FAIL is characterized. | Accepted | 8 |
| [0038](0038-captured-threshold-audit.md) | Captured-threshold audit (extends ADR-0031): record defensibility is orthogonal to pass/fail; `overall_conformant=True` means capture-complete, NOT all-pass. Liability taxonomy: UNTETHERED/UNCAPTURED/SELF_CONFIRMING (numeric-only; bool==expected is a pass)/UNSOURCED. Hardened exemption: `characterized` requires a captured note. Check 7 retired via `"expected": True` capture, not characterization; T1_R2 orientation FAILURE is captured but reversal is ⚠ UNVERIFIED. | Accepted | 6 |
