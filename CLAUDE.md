...
- [ ] **Reconcile — IN PROGRESS (first pass done; footprint work pending)** — ran `reconcile()` on sha `50d1f143` vs EDR_T1/R2/confidence. Raw numbers:
  - **rugosity −3.3%** (1.368 vs 1.415): close, but rugosity is global 3D/2D area and is footprint-extent-sensitive — non-belt AOI (aspect 2.674) means this is footprint-contaminated, not a clean confirmation.
  - **VRM −13.5% raw** (0.066 vs 0.076): SIGN NOT YET VERIFIED. ADR-0032 found our Python VRM +14% HIGH vs MultiscaleDTM on the T1 area-survey DSM. If the same offset applies here, the impl-corrected direction is OPPOSITE and the real surface gap is ~−24% (ours locally smoother → genuine sparse-coverage fidelity finding, NOT implementation noise). **Must run Python VRM on a P13HMEON reference TIF (T3/C1 or T3/R1) and compare to published value to confirm sign before interpreting.**
  - **mean_elevation +144%** (0.591 vs 0.242 m): NOT a valid 1:1. Our DSM spans 1.275 m Z relief vs published 0.670 m. Cause: GATE#6 bypassed (aspect 2.674 < 5.0 threshold) — out-and-back geometry + 131/272 partial registration → non-belt footprint oriented/located across cross-slope relief rather than along published single-pass transect. Footprint must be matched to published transect before extent-sensitive metrics (rugosity, mean_elev) are interpreted.
  - **Next pass:** (1) confirm VRM offset sign via P13HMEON TIF roundtrip; (2) diagnose relief gap — is our 10×1 m belt crossing the slope or running along it vs published?; (3) match footprint to published single-pass transect and re-reconcile. Then STOP for review.
