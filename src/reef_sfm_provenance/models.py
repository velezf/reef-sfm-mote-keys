...
# --------------------------------------------------------------------------- #
# Quality control — three-state (used by qc/checker.py)
# --------------------------------------------------------------------------- #
...
class QCCheck(BaseModel):
    ...
    name: str
    description: str
    target: Optional[str] = Field(None, description='Human-readable, e.g. "<= 0.3 px (Toth S2)".')
    units: Optional[str] = None
...
class QCResult(BaseModel):
    ...
    units: Optional[str] = None
    message: str = ""
    source: Optional[str] = Field(None, description="Manifest field the check read.")
    skipped_reason: Optional[str] = Field(
        None, description="Set when status is NOT_EVALUABLE: why the precondition was absent."
    )
...
class QCReport(BaseModel):
    ...
    def summary(self) -> dict[str, int]:
        out = {s.value: 0 for s in QCStatus}
    ...
    def passed(self) -> bool:
        """A run passes only if there are no FAILs. NOT_EVALUABLE does not pass for you."""
        return all(r.status is not QCStatus.FAIL for r in self.results)
